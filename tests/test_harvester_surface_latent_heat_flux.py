...
def test_gridcell_min_max(tolerance=0.001):
    ...
    python code
    """
    offline_min = -45.172398
    offline_max = 726.7792 
    for i, harvested_tuple in enumerate(data1):
        if harvested_tuple.statistic == 'maximum':
            ...
            """In order to get the tolerance assert tests to work
               the values need to be positive.
            """   
            minimum        = -minimum
            harvested_min  = harvested_tuple.value    
            harvested_min  = -harvested_min
            assert minimum <= (1 + tolerance) * harvested_min
            assert minimum >= (1 - tolerance) * harvested_min
          
            offline_min         = -offline_min
            assert offline_min <= (1 + tolerance) * harvested_min
            assert offline_min >= (1 - tolerance) * harvested_min
