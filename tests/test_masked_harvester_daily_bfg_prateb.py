...
def test_global_mean_values(tolerance=0.001):
    ...
    for harvested_data in data1[0]: 
        ...
        if has_region:
           global_means = [1.768056169059557e-05,4.9329753568756415e-06, 5.38787458479942e-06, \
                           6.123624870575077e-06, 3.459784651047802e-06, 5.755749727687249e-06, \
                           1.5560423627941472e-05]
...
def test_variance_values(tolerance=0.001):
    ...
    for harvested_data in data1[0]:  
        has_region = any(hasattr(data, 'regions') for data in data1) #Returns a boolean.
        if has_region:
           variances = [3.898239874511611e-09, 2.498356432095225e-09, 1.7000085928019072e-09, \
                        3.570127301910865e-09, 1.6422440760873192e-09, 2.4185797448065218e-09, \
                        8.85632744690667e-09] 
           for i, harvested_tuple in enumerate(data1):
...
def test_gridcell_max(tolerance=0.001):
    ...
    for harvested_data in data1[0]:  
        has_region = any(hasattr(data, 'region') for data in data1) #Returns a boolean.
        if has_region:
           max_values = [0.0007065914, 0.0018657173, 0.0013675715, \
                         0.0018657173, 0.0013675715, 0.0018657173, \
                         0.0018657173] 
